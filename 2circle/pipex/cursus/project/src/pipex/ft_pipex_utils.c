...
/*   Updated: 2025/08/03 23:09:20 by inhlee           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*access_cmd(char *cmd, char **envp)
{
	...
	ex_path = ft_split(find_path(envp), ':');
	while (ex_path[i])
	{
		...
		if (access(ans_excute, F_OK | X_OK) == 0)
			return (ft_frees(ex_path), ans_excute);
		i++;
		free(ans_excute);
	}
	return (ft_frees(ex_path), NULL);
}
...
void	cmd_excute(char *argv, char **envp)
{
	...
	excute_path = access_cmd(cmd_split[0], envp);
	if (!excute_path)
	{
		ft_frees(cmd_split);
		write(2, "Command not found\n", 18);
